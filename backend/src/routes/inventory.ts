...
*/
  app.get('/', async (request, reply) => {
      const userInventory = await db.select().from(inventory).where(eq(inventory.userId, request.session.user.id)).orderBy(desc(inventory.currentPrice))
  ...
  */
  ...
  app.patch<{ Params: { id: string } }>('/:id/fetch-price', async (request, reply) => {
    const item = await db.select().from(inventory).where(and(eq(inventory.id, request.params.id), eq(inventory.userId, request.session.user.id))).limit(1)
    if (!item.length)
      throw app.httpErrors.notFound('Item not found')
    const price = await priceFetch(item[0].url)
    if (!price)
      throw app.httpErrors.badRequest('Could not fetch price')
